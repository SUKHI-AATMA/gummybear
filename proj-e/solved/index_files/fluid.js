...
function Goo() {
    ...
    function l(n) {
        TweenMax.killAll(), i = ismobile ? 0 : .16 * view.size.width > 211 ? Math.floor(.16 * view.size.width) : 211, end = ismobile ? 50 : 90, t = ismobile ? Math.ceil((view.size.width - i - end) / 2) : Math.ceil((view.size.width - i - end) / 3), o = ismobile ? Math.ceil(view.size.height / 3) : Math.ceil(.5 * view.size.height);
        var a = "down" == n ? o : 0;
        s = [
            [0, a],
            [.25 * t, a],
...
var s, w, h, os, mainvideo, myScroll, isScrolling = !1,
    ...
    m99 = {
        init: function() {
            var e = this;
            os = $(window).scrollTop(), $(window).resize(function() { e.resize() }).resize(), $(window).scroll(function() { e.scroll() }).scroll(), e.activate(), /*e.menu.init(),*/ $("body").removeClass("hold"), $(".up").click(function() { window.scrollTo(0, 200), $("html, body").stop().animate({ scrollTop: 0 }, 333, "swing") }), window.onpopstate = function() { e.open({ url: document.location.href, title: "", ID: "" }) }
        },
        resize: function() { 
        	w = window.innerWidth, h = window.innerHeight, this.adjust()
        },
        adjust: function() { 
        	mobile.detect() ? ($(".project .visual > div div.video-perspective").css({ perspective: $(".project").outerWidth() - 30, height: .75 * $(".project").outerWidth() }), $(".grid").css("height", h), $("footer").css("height", .66667 * h)) : ($(".project").css({ height: 2 * h, "margin-bottom": .5 * h }), $(".project .visual").css({ width: .6666 * $(".project").outerWidth(), bottom: .5 * h - .5 * $(".project").outerWidth() * .7 + .5 * $(".project").outerWidth() * .3, "margin-left": .3333 * $(".project").outerWidth() }), $(".project .visual > div div.video-perspective").css({ perspective: .6666 * $(".project").outerWidth() - 30, height: .6666 * $(".project").outerWidth() * .75 }), $(".cover").css("height", h), $("footer, .cover-wrapper, .intro").css("height", .5 * h), $(".grid").css("height", "")) },
        scroll: function() {
            var e = this;
        ...
        halt: function() { 
        	// window.cancelAnimationFrame(int), int = null, int = null, $(".skew").css("transform", "skewY(0deg)"), $(".skew canvas").css("transform", "translateY(0)")
        	 },
        glide: function() {
            if (s > os + 40 ? $("body").addClass("asce").removeClass("desc") : $("body").addClass("desc").removeClass("asce"), s > 200 ? $("header .up").addClass("show") : $("header .up").removeClass("show"), detectEl($(".random")) && $(".random > li").each(function() { s > $(this).offset().top - h + 200 && $(this).addClass("on") }), !mobile.detect()) {
